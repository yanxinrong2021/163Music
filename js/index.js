...
(async function () {
    const baseBox = document.querySelector('.header-box .base'),
        ...
        playerButton = document.querySelector('.player-img'),
    ...
    const playend = function playend() {
        ...
        matchNum = 0
        middleImage.className = 'middle-img'
    }
    ...
    /* 点击按钮控制音乐播放 */
    const hammer=new Hammer(headerBox)
    hammer.on('tap',function(){
        middleImage.className = 'middle-img move'
        if(audioBox.paused){
            playerButton.style.display='none'
            middleImage.style.animationPlayState = "running";
            audioBox.play()
            handle()
            ...
            middleImage.style.animationPlayState = "paused"
            audioBox.pause()
            clearInterval(timer)
            timer = null
        }  
